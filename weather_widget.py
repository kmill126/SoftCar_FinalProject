...
def weather_widget():
	...
	'''
	# This is just to show how to get the value out of the input box
	# it often.get() will just need to be incorporated into the final function
	...
	# def recurring_run():
	# 	p1 = Process(target=grab_often, args=())
	# 	p1.start()

	def continuous_check():
		often_value = often.get()
		phone_number = num.get()
		recurring_run(often_value, phone_number)
	...
	exit_button = Button(left, text = "Quit", command = close_out, height=50)
	exit_button.pack()
...
# def recurring_run(often_value, phone_number):
# 	p1 = Process(target=grab_often, args=(often_value, phone_number))
# 	p1.start()
